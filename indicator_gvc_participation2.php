...
foreach($exp_country as $expData){
    foreach($year as $yearData){
        ...
        $value = $db->query($sql)->fetchAll();
        $finalsum = array_merge($finalsum,$value);
        }
        if($sectorZero == 1){
...
for($i=0;$i< count($final1);$i++){
    ...
    $result[$i]['value'] = round($final1[$i][0]/$finalsum[$i][0]*100,2);
...
for($j=0;$j< count($final2);$j++){
    ...
    $result[$j+count($final2)]['value'] = round($final2[$j][0]/$finalsum[$j][0]*100,2);
...
for($j=0;$j< count($final3);$j++){
    ...
    $result[$j+count($final3)+count($final2)]['value'] = round($final3[$j][0]/$finalsum[$j][0]*100,2);
    $result[$j+count($final3)+count($final2)]['year'] = $final3[$j][4];
    $result[$j+count($final3)+count($final2)]['indicator'] = 'GVC_part';
}
...
for($j=0;$j< count($final1A);$j++){
   $result[$j+$sumx]['exp_country'] = $final1A[$j][1];
    ...
    $result[$j+$sumx]['value'] = round($final1A[$j][0]/$finalsumA[$j][0]*100,2);
