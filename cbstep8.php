...
for($i=0; $i<count($country_data);$i++){
  ...
  where (variable = 'DVA_INTrex1' or variable='DVA_INTrex2' or variable='DVA_INTrex3')  and ( imp_country NOT IN ('sea', 'nca', 'sswa', 'enea', 'pac', 'ap', 'euz', 'eur', 'apta', 'saarc', 'nafta', 'mercosur', 'cptpp', 'rcep', 'apec', 'lac', 'pac_alliance', 'fealac', 'bimstec', 'wld'))    group by imp_country" ;
...
for($i=0; $i<count($country_data);$i++){
    ...
    $tableName = strtolower($country_data[$i]) . "_" . $year;
    $sql  = "select sum(value) as sum, exp_sector  from " . $tableName . " where (variable = 'DVA_INTrex1' or variable='DVA_INTrex2'  or variable='DVA_INTrex3' ) and ( imp_country NOT IN ('sea', 'nca', 'sswa', 'enea', 'pac', 'ap', 'euz', 'eur', 'apta', 'saarc', 'nafta', 'mercosur', 'cptpp', 'rcep', 'apec', 'lac', 'pac_alliance', 'fealac', 'bimstec', 'wld')) group by exp_sector" ;
    $value1 = $db->query($sql)->fetchAll();
...
// echo "\n**********12********\n";
//   echo json_encode($result);
    $set12 = json_encode($result);
//   $db->update("country_brief",["set12"=>$dataInput],["AND"=>["economy"=>$exp_country,"year"=>$year]]);
...
$result2 = array();
  ...
  where variable = 'total_export' and ( imp_country NOT IN ('sea', 'nca', 'sswa', 'enea', 'pac', 'ap', 'euz', 'eur', 'apta', 'saarc', 'nafta', 'mercosur', 'cptpp', 'rcep', 'apec', 'lac', 'pac_alliance', 'fealac', 'bimstec', 'wld'))" ;
   $value2 = $db->query($sql)->fetchAll();
   $result2['total'] += round($value2[0]['sum'],2);
    }
    $result['Agriculture']['value'] =round($result['Agriculture']['value'],2);
    $result['Agriculture']['ratio'] =round($result['Agriculture']['value'] /$result2['total']*100,2);
    $result['Mining']['value'] =round($result['Mining']['value'],2);
    $result['Mining']['ratio'] =round($result['Mining']['value'] /$result2['total']*100,2);
    $result['Low tech']['value'] =round($result['Low tech']['value'],2);
    $result['Low tech']['ratio'] =round($result['Low tech']['value'] /$result2['total']*100,2);
    $result['High and medium tech']['value'] =round($result['High and medium tech']['value'],2);
    $result['High and medium tech']['ratio'] =round($result['High and medium tech']['value'] /$result2['total']*100,2);
    $result['Utilities']['value'] =round($result['Utilities']['value'],2);
    $result['Utilities']['ratio'] =round($result['Utilities']['value'] /$result2['total']*100,2);
    $result['Construction']['value'] =round($result['Construction']['value'],2);
    $result['Construction']['ratio']=round($result['Construction']['value'] /$result2['total']*100,2);
    $result['Trade and repair service']['value'] =round($result['Trade and repair service']['value'],2);
    $result['Trade and repair service']['ratio']=round($result['Trade and repair service']['value'] /$result2['total']*100,2);
    $result['Tourism']['value'] =round($result['Tourism']['value'],2);
    $result['Tourism']['ratio']=round($result['Tourism']['value'] /$result2['total']*100,2);
    $result['Transport service']['value'] =round($result['Transport service']['value'],2);
    $result['Transport service']['ratio']=round($result['Transport service']['value'] /$result2['total']*100,2);
    $result['ICT service']['value'] =round($result['ICT service']['value'],2);
    $result['ICT service']['ratio']=round($result['ICT service']['value'] /$result2['total']*100,2);
     $result['Financial service']['value'] =round($result['Financial service']['value'],2);
    $result['Financial service']['ratio']=round($result['Financial service']['value'] /$result2['total']*100,2);
    $result['Property service']['value'] =round($result['Property service']['value'],2);
    $result['Property service']['ratio']=round($result['Property service']['value'] /$result2['total']*100,2);
    $result['Public and welfare service']['value'] =round($result['Public and welfare service']['value'],2);
    $result['Public and welfare service']['ratio']=round($result['Public and welfare service']['value'] /$result2['total']*100,2);
    $result['Private household service']['value'] =round($result['Private household service']['value'],2);
    $result['Private household service']['ratio']=round($result['Private household service']['value'] /$result2['total']*100,2);
//       echo "\n**********12A********\n";
//   echo json_encode($result);
    $set12a = json_encode($result);
//   $db->update("country_brief",["set12a"=>$dataInput],["AND"=>["economy"=>$exp_country,"year"=>$year]]);


$db->update("country_brief",["set11"=>$set11,"set11a"=>$set11a, "set12"=>$set12, "set12a"=>$set12a],["AND"=>["economy"=>$exp_country,"year"=>$year]]);
?>