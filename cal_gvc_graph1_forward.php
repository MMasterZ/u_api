...
//backward
$sql  = "select sum(value) as sum, exp_sector from " . $tableName . " where exp_country='" . $exp_country. "' and year = " . $year ." and (variable = 'DVA_INTrex1' or variable='DVA_INTrex2' or variable='DVA_INTrex3')  group by exp_sector order by sum DESC limit 5" ;
...
for($i=0;$i<count($backward);$i++){
    ...
    //backwar-layer-2
    $sql  = "select sum(value) as sum,  imp_country from " . $tableName . " where exp_country='" . $exp_country. "' and year = " . $year . " and exp_sector = '" . $sector ."' and (variable = 'DVA_INTrex1' or variable='DVA_INTrex2' or variable='DVA_INTrex3')   group by imp_country order by sum DESC limit 5" ;
    ...
    $result[$i]['exp_country1'] = $backward2[0]['imp_country'];
    $result[$i]['val1'] = round($backward2[0]['sum']/1000,2);
    $result[$i]['exp_country2'] = $backward2[1]['imp_country'];
    $result[$i]['val2'] = round($backward2[1]['sum']/1000,2);
    $result[$i]['exp_country3'] = $backward2[2]['imp_country'];
    $result[$i]['val3'] = round($backward2[2]['sum']/1000,2);
    $result[$i]['exp_country4'] = $backward2[3]['imp_country'];
    $result[$i]['val4'] = round($backward2[3]['sum']/1000,2);
    $result[$i]['exp_country5'] = $backward2[4]['imp_country'];
