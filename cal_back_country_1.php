...
where imp_country = '". $imp_country ."' and (variable = 'fva_fin_yl' or variable='fva_int_yl' )  group by source_country" ;
$value = $db->query($sql)->fetchAll();

} else {
 $sql  = "select sum(value) as sum,source_country,exp_sector,  imp_country  from 
" . $tableName . " 
where exp_sector = '" . $sector_data[$sector] . "' and imp_country = '". $imp_country ."' and (variable = 'fva_fin_yl' or variable='fva_int_yl' )  group by source_country" ;
$value = $db->query($sql)->fetchAll(); 
}
